...
class CharacterDescription:
    def __init__(
        ...
        model: Optional[CharacterDecompositionModel] = None,
    ):
        if model is not None:
            self.__name = model.name

            decompositions = tuple(
                DecompositionDescription(structureModel)
                for structureModel in model.structureSet
            )
    ...
    def prepareStructures(self, structureConverter: StructureConverter):
        ...
        self.__structures = tuple(
            structureConverter.convert(decomposition.node, decomposition.font)
            for decomposition in self.__decompositions
        )


class CharacterDecompositionSet:
    def __init__(self, model: CharacterDecompositionSetModel):
        self.__charDescs = tuple(
            CharacterDescription(model=decompositionModel)
            for decompositionModel in model.decompositionSet
        )
