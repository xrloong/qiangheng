...
class ContainerCJCodeHelper(CJCodeHelper):
    def __init__(self, outerLump, innerLump):
        [self.outerFrontCode, self.outerContainerCode, self.outerInteriorCode] = (
            outerLump.getXCode()
        )
        [self.innerFrontCode, self.innerContainerCode, self.innerInteriorCode] = (
            innerLump.getXCode()
        )
...
class CJCodeInfoEncoder(CodeInfoEncoder):
    ...
    def convertCodeInfoListToRadixList(direction, codeInfoList):
        ...
        for tmpCodeInfo in codeInfoList:
            tmpRadixList = CJCodeInfoEncoder.computeLumpListInDirection(
                direction, tmpCodeInfo
            )
