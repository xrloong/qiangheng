...
def main():
    ...
    for line in open(filename):
        ...
        if sre:
            ...
            newStrokeInfo = "{0};{1[0]:02X}{1[1]:02X}{1[2]:02X}{1[3]:02X};{2}".format(
                name, strokeComputer.getNewScope(), strokeComputer.getInfo()
            )
            print(
                re.sub(
                    pattern,
                    '\\1資訊表示式="(%s)%s" 筆劃資訊="%s"\\4'
                    % (name, strokeComputer.getNewExp(), newStrokeInfo),
                    l,
                )
            )
        else:
...
class BaseCurveComputer:
    ...
    def get_圓(self, startPoint, a, b):
        ...
        return [
            ...
            (True, topRightPoint),
            (False, rightPoint),
            (True, bottomRightPoint),
            (False, bottomPoint),
            (True, bottomLeftPoint),
            (False, leftPoint),
            (True, topLeftPoint),
            (False, topPoint),
        ]
    ...
    def get_左(self, startPoint, width):
        assert width > 0
        return [
            (False, (startPoint[0] - width, startPoint[1])),
        ]

    def get_上(self, startPoint, height):
        assert height > 0
        return [
            (False, (startPoint[0], startPoint[1] - height)),
        ]
    ...
    def get_趯(self, startPoint, width, height):
        assert width > 0 and height > 0
        return [
            (False, (startPoint[0] - width, startPoint[1] - height)),
        ]
    ...
    def get_臥捺(self, startPoint, w, h):
        ...
        midPoint1 = [
            startPoint[0] + halfW // 2 + halfH // 4,
            startPoint[1] + halfH // 2 - halfW // 4,
        ]
        midPoint2 = [
            cPoint[0] + halfW // 2 - halfH // 4,
            min(0xFF, cPoint[1] + halfH // 2 + halfW // 4),
        ]
        return [
            (True, midPoint1),
            (False, cPoint),
    ...
    def get_撇鉤之撇(self, startPoint, w, h):
        assert w > 0 and h > 0
        return [
            (True, (startPoint[0], startPoint[1] + h)),
    ...
    def get_斜鉤之斜(self, startPoint, w, h):
        assert w > 0 and h > 0
        return [
            (True, (startPoint[0] + w // 5, startPoint[1] + h * 4 // 5)),
            (False, (startPoint[0] + w, startPoint[1] + h)),
        ]
    ...
    def get_曲(self, startPoint, cr):
        assert cr > 0
        return [
            (True, (startPoint[0], startPoint[1] + cr)),
...
class CurveComputer_點(BaseCurveComputer):
    def checkType(self):
        assert self.actionList == ["0000", "0001"], "Error: {0}".format(
            self.getErrorInfo()
        )
...
class CurveComputer_橫鉤(BaseCurveComputer):
    def checkType(self):
        assert self.actionList == [
            "0000",
            "0001",
            "0001",
        ], "Error: {0}".format(self.getErrorInfo())
...
class CurveComputer_橫撇橫折鉤(BaseCurveComputer):
    def checkType(self):
        ...
        mid2X, mid2Y = self.getPos(pointList[-2])
        endX, endY = self.getPos(pointList[-1])
        assert startX < endX and startY == mid1Y, "Error: {0}".format(
            self.getErrorInfo()
        )
...
class CurveComputer_斜鉤(BaseCurveComputer):
    def checkType(self):
        assert self.actionList == ["0000", "0002", "0001", "0001"], "Error: {0}".format(
            self.getErrorInfo()
        )
