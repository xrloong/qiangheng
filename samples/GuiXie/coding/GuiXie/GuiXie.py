...
def computeStrokes(genre: GXGenre, lumps):
    if genre == GXGenre.Zhong:
        ...
        lump2 = lumps[1]
        strokes = (
            lump1.computeStrokesOnMainDiagonal() + lump2.computeStrokesOnMainDiagonal()
        )
        ...
        lump2 = lumps[1]
        strokes = (
            lump1.computeStrokesOnAntiDiagonal() + lump2.computeStrokesOnMainDiagonal()
        )
...
class GXCodeInfo(CodeInfo):
    ...
    def computeCode(self):
        return "{}/{}{}".format(
            self.computeGenreCode(), self.computeBodyCode(), self.computeRectCountCode()
        )
